...
const int THROUGHPUT = 2;   // vehicles cleared per second of green (comparison model)
...
// Formula : T_i = (V_i / V_total) * T_CYCLE   clamped to [MIN_GREEN, MAX_GREEN]
// Complexity: O(n log n) – sort dominates; allocation pass is O(n)
class GreedyScheduler {
public:
    static std::vector<SchedEntry> build(const std::vector<Road>& roads) {
        int total = 0;
        for (int i = 0; i < (int)roads.size(); i++) total += roads[i].vehicles;
        if (total == 0) total = 1;

        std::vector<int> idx(roads.size());
        for (int i = 0; i < (int)idx.size(); i++) idx[i] = i;
        // O(n log n) sort — highest traffic gets served first
        std::sort(idx.begin(), idx.end(), [&](int a, int b){
            return roads[a].vehicles > roads[b].vehicles;
        });

        std::vector<SchedEntry> sched;
        for (int rank = 0; rank < (int)idx.size(); rank++) {
            int i  = idx[rank];
            int vi = roads[i].vehicles;
            int t  = (int)((float)vi / total * T_CYCLE + 0.5f);
            t = clamp(t, MIN_GREEN, MAX_GREEN);
            SchedEntry e;
            e.roadIdx = i; e.vehicles = vi;
            e.greenTime = t; e.rank = rank;
            sched.push_back(e);
        }
        return sched;
    }

    // Green-time utilisation: % of total green seconds given to roads with vehicles
    static int efficiency(const std::vector<Road>& roads,
        ...
        for (int i = 0; i < (int)sched.size(); i++) {
            ...
            if (roads[sched[i].roadIdx].vehicles > 0) usefulTime += sched[i].greenTime;
        }
...
std::string badge(Sig s) {
    if (s == Sig::G) return BGGRN + BOLD + " G " + RST;
    if (s == Sig::Y) return BGYLW + BOLD + " Y " + RST;
...
void printView(const std::vector<Road>& roads,
               const std::vector<SchedEntry>& sched,
               int pi, int timer, int cycle, int cleared)
{
    std::cout << "\n"
        ...
        << "  ----[+]----  "
        << roads[2].name << " " << badge(roads[2].signal)
        << " " << roads[2].vehicles << "v\n"
        << "                    |\n"
        << "                " << BOLD << roads[1].name << RST
        << "  " << badge(roads[1].signal) << "  " << roads[1].vehicles << "v\n\n";

    std::cout << BOLD << "  Cycle: "       << YLW << cycle   << RST
              << BOLD << "   Cleared: "    << GRN << cleared << RST
              << BOLD << "   Efficiency: " << CYN
    ...
    for (int r = 0; r < (int)sched.size(); r++) {
        ...
        std::string t = act ?
            (GRN + std::to_string(timer) + "s" + RST) :
            (std::to_string(s.greenTime) + "s");
        std::cout << "  | " << p << std::setw(2) << (r+1) << e
                  << " | " << p << std::left << std::setw(8)
                  << roads[s.roadIdx].name << e
                  << " |    " << p << std::right << std::setw(3)
                  << s.vehicles << "     " << e
                  << "| " << std::setw(4) << t << "    "
                ...
                const std::vector<SchedEntry>& sched, int cycle)
{
    ...
    std::cout << "\n" << BOLD + MAG
              << "  === GREEDY SCHEDULE (Cycle " << cycle << ") ===\n" << RST
              << MAG << "  T_i = (V_i / " << total << ") x " << T_CYCLE
              << "s  clamp[" << MIN_GREEN << "," << MAX_GREEN << "]\n" << RST;

    for (int i = 0; i < (int)sched.size(); i++) {
        const SchedEntry& s = sched[i];
        float pct = (float)s.vehicles / total * 100.0f;
        std::cout << "  #" << (s.rank+1) << " " << BOLD
                  << std::left << std::setw(8) << roads[s.roadIdx].name << RST
                  << " | " << std::right << std::setw(2) << s.vehicles
                  << "v (" << std::fixed << std::setprecision(1) << pct << "%)"
...
class TrafficSystem {
    ...
    void allRed() {
        for (int i = 0; i < (int)roads.size(); i++) roads[i].signal = Sig::R;
    }

    void rebuild() {
        if (cycle > MAX_CYCLES) { done = true; return; }
    ...
    void discharge(int idx) {
        ...
        if (n < 1 && roads[idx].vehicles > 0) n = 1;
        if (n > roads[idx].vehicles) n = roads[idx].vehicles;
...
public:
    TrafficSystem() {
        ...
        std::cout << "\n" << GRN
                  << "  Starting simulation (" << MAX_CYCLES << " cycles)...\n"
                  << RST;
        sleepMs(1000);
        rebuild();
    }
    ...
    void printFinalReport() {
        ...
        for (int i = 0; i < (int)tmpR.size(); i++) tmpR[i].vehicles = tmpR[i].originalInput;
        ...
        std::cout << "\n" << BOLD + MAG
                  ...
                  << "  Fixed-time  : every road gets " << fixedT
                  << "s  (= T_cycle / " << NUM_ROADS << ")\n"
        ...
        for (int i = 0; i < NUM_ROADS; i++) {
            ...
            std::cout << "  | " << std::left  << std::setw(8) << roads[i].name
                      ...
                      << " | " << fixedT << "s -> " << std::setw(3) << fC << "v cleared "
